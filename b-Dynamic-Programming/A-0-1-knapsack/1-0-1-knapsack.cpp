...
int recursiveSolve(vector<int> value, vector<int> weight, int capacity, int n){
  ...
  if(weight[n-1] <= capacity){
    return max(
      ...
      // include
      value[n-1]+recursiveSolve(value, weight, capacity-weight[n-1], n-1), 
...
int memoSolve(vector<int> value, vector<int> weight, int capacity, int n){
  ...
  if(weight[n-1] <= capacity){
    return t1[n][capacity] = max(
      value[n-1]+memoSolve(value, weight, capacity-weight[n-1], n-1), 
...
int topDownSolve(vector<int> value, vector<int> weight, int capacity, int n){
  ...
  for(int i = 0; i < n+1; i++){
    for(int j = 0; j < capacity+1; j++){
      if(weight[i-1] < j){
        t2[i][j] = max(value[i-1]+t2[i-1][j-weight[i-1]], t2[i-1][j]);
      }else{
        t2[i][j] = t2[i-1][j];
      }
    }
